...
public class NewDownloadWindow extends JDialog implements ActionListener, DocumentListener {
  ...
  public NewDownloadWindow(Metadata metadata, String fileName, String folderPath) {
    initUI();
    this.metadata = metadata;
    if (this.metadata == null) {
      txtURL.setText(getUrlFromClipboard());
      txtURL.setCaretPosition(0);
    } else {
      txtURL.setText(metadata.getPrimaryUrl());
      txtURL.setCaretPosition(0);
    }
  ...
  private void createDownload(boolean now) {
    String urlStr = txtURL.getText();
    if (StringUtils.isNullOrEmptyOrBlank(urlStr)) {
