  ...
  private void loadQueues() {
    ...
    try (BufferedReader reader =
      ...
      for (int i = 0; i < count; i++) {
        ...
        for (int j = 0; j < c; j++) {
          queue.getQueuedItems().add(Long.parseLong(XDMUtils.readLineSafe(reader).trim()));
        }
        ...
        if (hasStartTime) {
          ...
          if (isPeriodic) {
            ...
            if (Integer.parseInt(reader.readLine()) == 1) {
              ...
              if (ln != null) queue.setExecDate(dateFormatter.parse(ln));
            }
          }
        }
  ...
  public void saveQueues() {
    ...
    try {
      ...
      for (int i = 0; i < count; i++) {
        ...
        ArrayList<Long> queuedItems = queue.getQueuedItems();
...
//  public void fixCorruptEntries(Iterator<Long> ids, XDMApp app) {
...
//    for (int i = 0; i < queueList.size(); i++) {
//      DownloadQueue q = queueList.get(i);
//      ArrayList<Long> corruptIds = new ArrayList<>();
//      for (int k = 0; k < q.getQueuedItems().size(); k++) {
//        var id = q.getQueuedItems().get(k);
