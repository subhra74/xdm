...
xdm.util = {
    ...
    isFirefox: function () {
        return chrome.runtime.getManifest().applications ? true : false;
    },
    getFileName: function (download) {
        var index = download.filename.lastIndexOf("/");
        if (index > 0) {
            return download.filename.substr(index + 1);
        }
        index = download.filename.lastIndexOf("\\");
        if (index > 0) {
            return download.filename.substr(index + 1);
        }
