package xdm.core.downloaders.metadata.manifests;
...
public class F4MManifest {
  private static final Logger logger = LoggerFactory.getLogger(F4MManifest.class);
  ...
  public ArrayList<String> getMediaUrls() throws Exception {
    ...
    if (fragNum >= fragCount) throw new Exception("No fragment available for downloading");
    ...
    while (fragNum < fragCount) {
      ...
      if (fragIndex >= 0) discontinuity = fragTable.get(fragIndex).discontinuityIndicator;
      else {
        // search closest
        for (int i = 0; i < fragTable.size(); i++) {
          if (fragTable.get(i).firstFragment < fragNum) continue;
      ...
      if (discontinuity != 0) {
        logger.info(
            "Skipping fragment " + fragNum + " due to discontinuity, Type: " + discontinuity);
        continue;
      }
  ...
  private void parseDoc(Document doc, String surl) throws XPathExpressionException {
    ...
    NodeList mediaNodeList =
        (NodeList) xpath.evaluate("/ns:manifest/ns:media", doc, XPathConstants.NODESET);
    ...
    for (int i = 0; i < mediaNodeList.getLength(); i++) {
      ...
      if (mediaFound) {
        ...
        media.setBootstrap(Base64.getDecoder().decode(bootstrapInfoStr));
        break;
      }
    }
  ...
  private int getSegmentFromFragment(int fragN) {
    if ((segTable.size() == 0) || (fragTable.size() == 0)) return 1;
    ...
    if (segTable.size() == 1) return firstSegment.firstSegment;
      ...
      for (int i = firstSegment.firstSegment; i <= lastSegment.firstSegment; i++) {
        if (segTable.size() >= (i - 1)) seg = segTable.get(i);
        else seg = prev;
  ...
  private void parseAsrtBox(byte[] asrt, int pos) {
    ...
    for (int i = 0; i < segCount; i++) {
      ...
      if ((segEntry.fragmentsPerSegment & 0x80000000L) > 0) segEntry.fragmentsPerSegment = 0;
      pos += 8;
  ...
  private void parseAfrtBox(byte[] afrt, int pos) {
    ...
    for (int i = 0; i < fragEntries; i++) {
      ...
      if ((fromTimestamp > 0)
          && (fragEntry.firstFragmentTimestamp > 0)
