			...
			while (true) {
				try {
					...
					if (currentTime - lastKeepAwakePing > 3000) {
						if (!App.getInstance().isAllFinished()) {
							PlatformUtils.keepAwakePing();
...
//						App.getInstance().notifyAppUpdate();
//						break;
//					case UpdateChecker.COMP_NOT_INSTALLED:
//						App.getInstance().notifyComponentInstall();
//						break;
//					case UpdateChecker.COMP_UPDATE_AVAILABLE:
//						App.getInstance().notifyComponentUpdate();
//						break;
//					}
				}
