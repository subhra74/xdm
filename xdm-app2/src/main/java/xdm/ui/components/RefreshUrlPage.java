...
import xdm.LinkRefreshCallback;
import xdm.App;
...
public class RefreshUrlPage extends Page implements LinkRefreshCallback {
	...
	private HttpDownloadMetadata md;
	...
	public void setDetails(HttpDownloadMetadata md) {
		this.md = md;
		if (StringUtils.isNullOrEmptyOrBlank(md.getOriginPage())) {
			btnOpenPage.setVisible(md.getHeaders().containsHeader("referer"));
		} else {
			btnOpenPage.setVisible(true);
		}
		System.out.println("ydlurl: " + md.getOriginPage());
		lblMonitoringTitle
				.setText(StringUtils.isNullOrEmptyOrBlank(md.getOriginPage())
	...
	public void showPanel() {
		super.showPanel();
		App.getInstance().registerRefreshCallback(this);
	}

	@Override
	public void close() {
		App.getInstance().unregisterRefreshCallback();
		super.close();
	}
...
//			wnd.launchWithUrl(md.getOriginPage());
		} else if (md.getHeaders().containsHeader("referer")) {
			PlatformUtils.browseURL(md.getHeaders().getValue("referer"));
		}
	}
}
