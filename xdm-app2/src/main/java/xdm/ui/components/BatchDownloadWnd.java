...
public class BatchDownloadWnd extends JFrame implements ActionListener {
	...
	public static List<String> getUrls() {
		...
		String text = PlatformUtils.getClipBoardText();
	...
	public BatchDownloadWnd(List<HttpDownloadMetadata> mdList) {
		fileExts = new HashSet<>();
		items = new BatchItem[mdList.size()];
		initUI();
		for (int i = 0; i < mdList.size(); i++) {
			HttpDownloadMetadata md = mdList.get(i);
			try {
	...
	private void createDownload(DownloadQueue q) {
		...
		for (int i = 0; i < model.size(); i++) {
			...
			if (item.selected) {
				...
				HttpDownloadMetadata metadata = item.metadata;
				folder = txtFile.getText();
				App.getInstance().createDownload(file, folder, metadata, false, q == null ? "" : q.getQueueId(), 0,
						0);
			}
		}
	}
