...
public class NewDownloadWindow extends JDialog implements ActionListener, DocumentListener {
	...
	public NewDownloadWindow(HttpDownloadMetadata metadata, String fileName, String folderPath) {
		initUI();
		...
		if (this.metadata.getUrl() != null) {
			...
			try {
				URL url = new URL(PlatformUtils.getClipBoardText());
	...
	private void createDownload(boolean now) {
		...
		App.getInstance().createDownload(file, filePane.getFolder(), metadata, now, queueId, 0, 0);
	}
