...
public class TrayHandler {
  ...
  public static void createTray() {
    ...
    if (PlatformUtils.detectOS() == PlatformUtils.LINUX) {
      if (Config.getInstance().isHideTray()) return;
      else {
            ...
            if ("ADD_URL".equals(name)) {
              ...
              if (urlList.size() > 0) {
                ...
                JOptionPane.showMessageDialog(
                    null, StringResource.get("LBL_BATCH_EMPTY_CLIPBOARD"));
              }
    ...
    CheckboxMenuItem monitoringItem =
        new CheckboxMenuItem(StringResource.get("BROWSER_MONITORING"));
    monitoringItem.addItemListener(
        new ItemListener() {
    ...
    popup.add(exitItem);
    trayIcon.setToolTip(App.XDM_WINDOW_TITLE);
