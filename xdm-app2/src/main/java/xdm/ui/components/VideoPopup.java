...
public class VideoPopup extends JDialog implements ActionListener, Comparator<VideoPopupItem> {
	...
	public synchronized void addVideo(HttpDownloadMetadata metadata, String file, String info) {
	...
	private void arrangeList() {
		...
		itemsCopy.addAll(App.getInstance().getVideoItemsList());
	...
	public synchronized void actionPerformed(ActionEvent e) {
		...
		for (VideoPopupItem item : App.getInstance().getVideoItemsList()) {
			if (name.equals(item.getMetadata().getId())) {
				collapse();
				HttpDownloadMetadata md = item.getMetadata().copy();
				Logger.log("dash metdata ? " + (md instanceof DashMetadata));
				App.getInstance().addVideo(md, item.getFile());
			}
		}
		if (name.equals("CLOSE")) {
			collapse();
			menuBox.removeAll();
			App.getInstance().getVideoItemsList().clear();
			dispose();
			_this = null;
	...
	private boolean addItem(VideoPopupItem item) {
		...
		for (int i = 0; i < App.getInstance().getVideoItemsList().size(); i++) {
			VideoPopupItem p = App.getInstance().getVideoItemsList().get(i);
			HttpDownloadMetadata m1 = item.getMetadata();
			HttpDownloadMetadata m2 = p.getMetadata();
		...
		App.getInstance().getVideoItemsList().add(item);
		return true;
	}
