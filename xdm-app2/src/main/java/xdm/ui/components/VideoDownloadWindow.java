...
import xdm.mediaconversion.MediaFormat;
...
public class VideoDownloadWindow extends JDialog implements ActionListener, DocumentListener {
	...
	public VideoDownloadWindow(HttpDownloadMetadata metadata, String file) {
		// this.folder = Config.getInstance().getDownloadFolder();
		this.metadata = metadata;
		if (this.metadata == null) {
			this.metadata = new HttpDownloadMetadata();
		}
...
//			}
	...
	private void createDownload(boolean now) {
		...
		App.getInstance().createDownload(file, filePane.getFolder(), metadata, now, queueId, fmtIndex,
				dashAudioOnly ? 1 : 0);
	}
