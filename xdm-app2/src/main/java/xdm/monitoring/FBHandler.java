	...
	public static boolean handle(File tempFile, ParsedHookData data) {
		try {
			...
			for (int i = 0; i < sdUrls1.size(); i++) {
				...
				App.getInstance().addMedia(metadata, file + ".mp4", "MP4 LOW");
			}
			for (int i = 0; i < sdUrls2.size(); i++) {
				...
				App.getInstance().addMedia(metadata, file + ".mp4", "MP4 MEDIUM");
			}
			for (int i = 0; i < hdUrls1.size(); i++) {
				...
				App.getInstance().addMedia(metadata, file + ".mp4", "MP4 HD");
			}
			for (int i = 0; i < hdUrls2.size(); i++) {
				...
				App.getInstance().addMedia(metadata, file + ".mp4", "MP4 HQ");
			}
			return true;
		} catch (Exception e) {
			Logger.log(e);
